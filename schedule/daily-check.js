...
function getAllPages(i, issue_number) {
	return octokit.rest.issues
		.listComments({ owner, page: i, per_page: 100, repo, issue_number })
		.then((res) => {
			if (res.data.length > 0) {
				return getAllPages(i + 1, issue_number).then(
					(comments) => res.data.concat(comments),
				);
			}
			return [];
		})
...
async function run(d, issue_number) {
	...
	if (problem && problem.issue_number) {
		...
		comments.forEach((comment) => {
			...
			if (
				mySolutions[login][d - 1] && mySolutions[login][d - 1].onTime !== void 0
			) {
